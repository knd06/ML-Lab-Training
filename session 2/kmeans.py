...
class Member:
    def __init__(self, r_d, label=None, doc_id=None):
        self._r_d = r_d  # tf-idf of d: text
...
class Kmeans:
    ...
    def stopping_condition(self, criterion, threshod):
        ...
        if criterion == 'max_iters':
            return True if self._iteration >= threshod else False
        elif criterion == 'centroid':
            E_new = [list(cluster._centroid) for cluster in self._clusters]
            E_new_minus_E = [centroid for centroid in E_new
                             if centroid not in self._E]
            self._E = E_new
            return True if len(E_new_minus_E) <= threshod else False
        else:
            new_S_minus_S = self._new_S - self._S
            self._S = self._new_S
            return True if new_S_minus_S < threshod else False
...
if __name__ == '__main__':
    K = Kmeans(20)
    K.load_data('session 1/data/20news-bydate/tfidf_full.txt')
    K.run(86, 'similarity', 1e-3)
